...
from .access_token import ServiceRtc, AccessToken
...
class RtcTokenBuilder:
    ...
    def build_token_with_user_account(
        ...
        service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_JOIN_CHANNEL, privilege_expire)
        if role == Role.PUBLISHER:
            service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_PUBLISH_AUDIO_STREAM, privilege_expire)
            service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_PUBLISH_VIDEO_STREAM, privilege_expire)
            service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_PUBLISH_DATA_STREAM, privilege_expire)
    ...
    def build_token_with_uid_and_privilege(
        ...
        to generate a new token, and then call renewToken to pass the new token to the SDK, or call joinChannel to
         re-join the channel.
                @note
        ...
        :param channel_name: The unique channel name for the Agora RTC session in string format. The string length must
         be less than 64 bytes. The channel name may contain the following characters:
        ...
        - "!", "#", "$", "%", "&", "(", ")", "+", "-", ":", ";", "<", "=", ".", ">", "?", "@", "[", "]", "^", "_", "
        {", "}", "|", "~", ",".
        :param uid: The user ID. A 32-bit unsigned integer with a value range from 1 to (2^32 - 1). It must be unique.
        Set uid as 0, if you do not want to authenticate the user ID, that is, any uid from the app client
        can join the channel.
        :param token_expire: represented by the number of seconds elapsed since now. If, for example, you want to
        access the
        ...
        :param join_channel_privilege_expire: The Unix timestamp when the privilege for joining the channel expires,
         represented
        by the sum of the current timestamp plus the valid time period of the token. For example, if you set
         join_channel_privilege_expire as the
        ...
        represented
        by the sum of the current timestamp plus the valid time period of the token. For example, if you set
        pub_audio_privilege_expire as the
        current timestamp plus 600 seconds, the token expires in 10 minutes. If you do not want to enable
        this privilege,
        ...
        represented
        by the sum of the current timestamp plus the valid time period of the token. For example, if you set
        pub_video_privilege_expire as the
        current timestamp plus 600 seconds, the token expires in 10 minutes. If you do not want to enable
        this privilege,
        set pub_video_privilege_expire as the current Unix timestamp.
        :param pub_data_stream_privilege_expire: The Unix timestamp when the privilege for publishing data streams
        expires, represented
        by the sum of the current timestamp plus the valid time period of the token. For example, if you set
         pub_data_stream_privilege_expire as the
        current timestamp plus 600 seconds, the token expires in 10 minutes. If you do not want to enable this
        privilege,
    ...
    def build_token_with_user_account_and_privilege(
        ...
        service_rtc = ServiceRtc(channel_name, account)
        service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_JOIN_CHANNEL, join_channel_privilege_expire)
        service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_PUBLISH_AUDIO_STREAM, pub_audio_privilege_expire)
        service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_PUBLISH_VIDEO_STREAM, pub_video_privilege_expire)
        service_rtc.add_privilege(ServiceRtc.K_PRIVILEGE_PUBLISH_DATA_STREAM, pub_data_stream_privilege_expire)
        token.add_service(service_rtc)
