...
def get_pre_signed_upload_url(bucket_name: str, key: str, access_key: str, secret_key: str, file_type: str, expiration):
    if file_type == MediaType.IMAGE:
        conditions = [['content-length-range', 0, MAX_IMAGE_FILE_SIZE], ['starts-with', '$Content-Type', 'image/']]
    else:
        conditions = [['content-length-range', 0, MAX_VIDEO_FILE_SIZE], ['starts-with', '$Content-Type', 'video/']]
